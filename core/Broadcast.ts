...
class Broadcast {
    /**
     * @param {Broadcastable} source Target to send the broadcast
     * @param {string} message
     * @param {number|boolean} wrapWidth=false width to wrap the message to or fon't wrap at all
     * @param {boolean} useColor Whether to parse color tags in the message
     * @param {?function(target, message): string} formatter=null Function to call to format to
     * message of each target
     */
    static at(
        source,
        message = '',
    ...
    static line(width, fillChar = '-', color = null) {
        ...
        return (
            openColor +
            new Array(width + 1).join(fillChar) +
            closeColor
        );
    }
    ...
    static wrap(message, width = 80) {
        return Broadcast._fixNewlines(
            wrap(ansi.parse(message), width)
        );
    }
    ...
    static isBroadcastable(source) {
        return (
            source && typeof source.getBroadcastTargets === 'function'
        );
    }
}
